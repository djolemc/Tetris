...
function setBoardSize() {
    ...
    let miniBoardSize = document.getElementById('nextPiece');
    boardSize.height = boardHeight;
    boardSize.width = boardWidth;

    miniBoardSize.height = miniBoardSize.width = SQ * MINI;
...
//draw a square
function drawSquare(canvas, x, y, color, strokeColor) {
...
function drawBoard() {
    for (r = 0; r < ROW; r++) {
        for (c = 0; c < COL; c++) {
            drawSquare(ctx, c, r, board[r][c], "#00cc99");
        }
    }
}
...
function drawMiniBoard() {
    for (r = 0; r < MINI; r++) {
        for (c = 0; c < MINI; c++) {
            drawSquare(nxt, c, r, miniBoard[r][c], 'black');
        }
    }
}
    ...
    for (r = 0; r < this.activeTetromino.length; r++) {
        for (c = 0; c < this.activeTetromino.length; c++) {
            ...
            if (this.activeTetromino[r][c]) {
                drawSquare(ctx,this.x + c, this.y + r, color, "#00cc99");
            }
        }
    }
};
...
Piece.prototype.drawNext = function (tetris, color) {
    for (r = 0; r < tetris.length; r++) {
        for (c = 0; c < tetris.length; c++) {
            if (tetris[r][c]) {
                drawSquare(nxt,c + 1, r + 1, color, "black");
            }
        }
    }
};
